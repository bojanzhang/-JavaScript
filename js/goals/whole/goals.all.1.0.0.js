...
//						Goals.exportJs(src, src_callback);
			...
			function export_css(src) {
				...
				if(src.startsWith('//')) {
					var url = window.location.href;
					if(url[4] === 's') {
						css = Goals.exportCss("https:" + src);
					}else {
						css = Goals.exportCss("http:" + src);
					}
				}else if(src.charAt(0) === '/') {
					css = Goals.exportCss(common.PROJECT_NAME + src);
				}else {
					css = Goals.exportCss(src);
				}
				_private.cssTags.push(css);
			}
...
;(function(Goals) {
	...
	var ajax_success = function(res, okFn) {
		if(Goals.prompt) {
			if(res.code !== 0) {
				Goals.prompt.hideLoading();
